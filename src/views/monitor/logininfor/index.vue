<template>
  <div class="page">
    <el-card>
      <div class="toolbar">
        <el-form inline>
          <el-form-item label="登录名">
            <el-input v-model="query.ipaddr" clearable style="width:140px" placeholder="IP/用户名" />
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="load">搜索</el-button>
            <el-button v-permission="['monitor:logininfor:remove']" type="danger" @click="onClean">清空</el-button>
          </el-form-item>
        </el-form>
      </div>
      <el-table :data="rows" border v-loading="loading">
        <el-table-column prop="infoId" label="ID" width="80" />
        <el-table-column prop="userName" label="用户名" width="120" />
        <el-table-column prop="ipaddr" label="IP" width="140" />
        <el-table-column prop="loginLocation" label="地点" min-width="100" />
        <el-table-column prop="browser" label="浏览器" width="100" />
        <el-table-column prop="os" label="系统" width="100" />
        <el-table-column prop="status" label="状态" width="90">
          <template #default="{ row }">
            <el-tag :type="row.status === '0' ? 'success' : 'danger'">{{ row.status === '0' ? '成功' : '失败' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="loginTime" label="时间" width="170" />
        <el-table-column label="操作" width="100">
          <template #default="{ row }">
            <el-button v-permission="['monitor:logininfor:remove']" size="small" type="danger" @click="onRemove(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
        class="pager"
        layout="total, prev, pager, next"
        :total="total"
        :page-size="query.pageSize || 10"
        :current-page="query.pageNum || 1"
        @current-change="onPage"
      />
    </el-card>
  </div>
</template>

<script setup>
import { reactive, ref, onMounted } from 'vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { listLogininfor, delLogininfor, cleanLogininfor } from '@/api/monitor/logininfor'

const loading = ref(false)
const rows = ref([])
const total = ref(0)
const query = reactive({ pageNum: 1, pageSize: 10, ipaddr: '' })

async function load() {
  loading.value = true
  try {
    const res = await listLogininfor({ ...query })
    rows.value = res?.rows || []
    total.value = Number(res?.total || 0)
  } finally {
    loading.value = false
  }
}
function onPage(p) { query.pageNum = p; load() }
async function onRemove(row) {
  await ElMessageBox.confirm('确认删除该登录日志？', '提示', { type: 'warning' })
  await delLogininfor(row.infoId)
  ElMessage.success('已删除')
  load()
}
async function onClean() {
  await ElMessageBox.confirm('确认清空全部登录日志？', '提示', { type: 'warning' })
  await cleanLogininfor()
  ElMessage.success('已清空')
  load()
}
onMounted(load)
</script>

<style scoped>
.page { padding: 16px; }
.toolbar { margin-bottom: 8px; }
.pager { margin-top: 12px; display: flex; justify-content: flex-end; }
</style>
